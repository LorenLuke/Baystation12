...
/datum/power			//Could be used by other antags too
	...
	var/list/verbpath // Path to a verb that contains the effects, or a list for multiple abilities.
	var/for_infested = 1 //0-no, 1- yes, 2- infested only
...
//		log_debug("[P] - [Pname] = [P.name == Pname ? "True" : "False"]")
	...
	if(!Thepower.isVerb && Thepower.verbpath)
		if(!islist(Thepower.verbpath))
			call(M.current, Thepower.verbpath)()
		else
			for(var/P in Thepower.verbpath)
				call(M.current, P)()
