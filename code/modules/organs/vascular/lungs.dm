/***************************************
Approximate hemoglobin dissociation sigmoid plot

x=02 pressure (torr)
y=oxy saturation


y=-20 + 120/(1+e**(-0.065*(x-25)))



****************************************/

#define EULERCONSTANT 2**(1/ln(2))

#define KPA_TO_TORR 7.50062
...
/obj/item/organ/internal/lungs
	...
	var/list/required_gasses=list("oxygen")
	var/list/exhale_type = list("carbon_dioxide")

	var/last_breath

	var/datum/gas_mixture/breath_in_lungs

	var/min_breath_pressure

	var/rupture_capacity = 6.6

	var/lung_pv= 0 // lung pressure times lung volume
	var/breathing = 0
	var/tidal_capacity = 0.5
	var/max_capacity = 6

	var/respiration_rate = 12





/obj/item/organ/internal/lungs/proc/inhale(var/held_breath = 0)
	if(!owner)
		return 1
	...
	var/breath_volume = tidal_capacity
	if(held_breath)
		breath_volume = max_capacity


	//Okay, we can breathe, now check if we can get air
	breath = get_breath_from_internal(breath_volume) //First, check for air from internals
	if(!breath)
		breath = get_breath_from_environment(breath_volume) //No breath from internals so let's try to get air from our location
...
///Breathe in...
	...
	var/breath_pressure = max(5,breath.total_moles*R_IDEAL_GAS_EQUATION*breath.temperature/breath_volume) // hard minimum 5 kpa to keep byond from shitting itself in space
	if(lung_pv/ pressure  > rand(95,105) * rupture_capacity/100) // lung rupture
		rupture()
	...
	for(var/G in breath,gas)
		var/gas = breath.gas[G]
		var/gas_kpa = breath_pressure * gas/breath.total_moles
		gases.add(gas = gas_kpa)

	lung_process_gasses(gasses)



/obj/item/organ/internal/lungs/proc/exhale(var/held_breath = 0)

	lung_internal_pressure = 5





/obj/item/organ/internal/lungs/proc/respire(datum/gas_mixture/breath, var/held_breath = 0)
	if(!owner)
		return 1
	if(!breath)
		return 1
...
///Breathe in...
	...
	if(breath_volume * last_pressure/breath_pressure > rand(95,105) * rupture_capacity/100) // lung rupture
		rupture()
