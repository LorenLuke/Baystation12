...
/datum/reagents/proc/process()
	...
	if(istype(src.my_atom, /mob/))
		var/mob/mob1 = my_atom
		if(istype(mob1, /mob/living/carbon/human))
			var/mob/living/carbon/human/M = mob1
			if(!src == M.vessel)
				src.ETE(M.vessel)
			var/cap1 = M.vessel.GHC()
			var/t1 = M.vessel.temperature
			var/list/returns = CETE(cap1, t1, cap1, M.bodytemperature)
			var/delta_e1 = returns[1]
			var/delta_e2 = returns[2]
			M.vessel.ATE(delta_e1)
			var/newbtemp = CATE(delta_e2, cap1, M.bodytemperature)
...
/*
		...
		if(istype(T, /turf/space))
			...
			//PLACEHOLDER!!! AHHHHHH! >.<"
//		else
//			temp_to = T:zone.air.temperature
...
datum/reagents/proc/trans_to(var/obj/target, var/amount=1, var/multiplier=1, var/preserve_data=1)//if preserve_data=0, the reagents data will be lost. Usefull if you use data for some strange stuff and don't want it to be transferred.
	...
	for (var/datum/reagent/current_reagent in src.reagent_list)
		...
		R.add_reagent(current_reagent.id, (current_reagent_transfer * multiplier), trans_data, safety = 1, R.temperature)	//safety checks on these so all chemicals are transferred
...
datum/reagents/proc/shock(var/atom/A)
	handle_reactions(shock=1)
...
datum/reagents/proc/handle_reactions2(var/shock)
	...
	for(var/datum/reagent/R in reagent_list) // Usually a small list
		for(var/reaction in chemical_reactions_list[R.id]) // Was a big list but now it should be smaller since we filtered it with our reagent id
			...
			if(C.requires_heating)
				if((temperature < C.temperature_lower_bound) || (temperature > C.temperature_upper_bound))
					continue

			if(C.requires_shock)
				if(!shock && (!prob(mixing)))
					continue
		...
		if(INJECT)
			for(var/datum/reagent/R in reagent_list)
				if(ismob(A) && R)
					spawn(0)
						if(!R) return
						else R.reaction_mob(A, INJECT, R.volume+volume_modifier)
	return

datum/reagents/proc/add_reagent(var/reagent, var/amount, var/list/data=null, var/safety = 0, var/temperature = src.temperature)
...
datum/reagents/proc/GTE() // Get Thermal Energy; returns total thermal energy in joules
...
datum/reagents/proc/GHC() // Get Heat Capacity NOT the atom, but the datum/reagents holder
	var/heat = 0 //heat required to heat the solution 1 degree
	for (var/datum/reagent/current_reagent in src.reagent_list)
		if (!current_reagent)
			continue
		var/heat_add = current_reagent.volume * current_reagent.density * current_reagent.specific_heat * (current_reagent.molarmass ** -1)
		//delta-T = ml * g/ml * joul/mol * mol/g
		//formula for heat capacity = volume * denisty * specific_heat * (molarmass ** -1)
		heat += heat_add
	return heat
...
datum/reagents/proc/ATE(var/amount)// Add thermal Energy; Add x energy, in joules (use negatives for endothermic)
	var/heat = 0 //heat required to heat the solution 1 degree
	for (var/datum/reagent/current_reagent in src.reagent_list)
		if (!current_reagent)
			continue
...
////reaction code
	...
	//					mixes_left = min(multipliers)

						if (C.requires_mixing)
							mixing -= C.requires_mixing
						if (!C.instant)
							sleep(20-(mixing**1.7))
						var/templist = react(C, shock, spark)
						C.on_reaction(src, templist[2])
