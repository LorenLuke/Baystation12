...
datum/reagent
	...
	var/molarmass = 1 // g/mol
	var/density = 1 // g/ml
	var/specific_heat = 1 // joules * (1/moles) * (1/specific heat) = 1 degree
	var/list/cc = list() //constituent_chemicals
//	var/formula = ""
...
datum/reagent/proc/CTE(var/volume, temperature = null) //Calculate thermal energy; Calculate thermal energy in joules
	var/thermal_energy = volume * density * specific_heat * (molarmass ** -1)
	if(temperature)
		thermal_energy *= temperature
	else
		thermal_energy *= src.holder.temperature

	return thermal_energy

/*
datum/reagent/proc/decompose(var/amount)
	var/vol = volume
	var/
	for(var/S in src.cc)
		var/multi = src.cc[S]
...
datum/reagent/water/carbonatedwater
	name = "Carbonated Water"
	id = "carbonatedwater"
	description = "Water with dissolved carbon dioxide"
	color = "#0064C8" // rgb: 0, 100, 200
...
datum/reagent/halon
	name = "Halon"
	id = "halon"
	description = "A chemical fire retardant for putting out raging conflagrations."
	reagent_state = LIQUID
	color = "#0064C8" // rgb: 0, 100, 200
...
datum/reagent/heroin
	name = "Space Heroin"
	id = "heroin"
	description = "The manliest drug there is."
	reagent_state = LIQUID
	color = "#60A584" // rgb: 96, 165, 132
	overdose = 30
	...
	on_mob_life(var/mob/living/M as mob)
		...
		if(isturf(M.loc) && !istype(M.loc, /turf/space))
			if(M.canmove && !M.restrained())
				if(prob(10)) step(M, pick(cardinal))
		if(prob(7)) M.emote(pick("twitch","shake","scream","chuckle","blink_r", "shiver"))
		return
			...
			if(ishuman(M))
				...
				if(H.glasses) //Doesn't protect you from the acid but can melt anyways!
					if(prob(meltprob) && !H.glasses.unacidable)
						H << "<span class='danger'>Your glasses melt away!</span>"
						del (H.glasses)
