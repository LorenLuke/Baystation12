...
obj/item/weapon/reagent_containers/throw_impact(var/atom/hit_atom)
	..()
	var/obj/item/weapon/reagent_containers/glass/beaker/noreact/N = new /obj/item/weapon/reagent_containers/glass/beaker/noreact //temporary holder
	N.volume = 1000

	src.reagents.copy_to(N, src.reagents.total_volume, safety = 1)
	sleep(2)
	if(!src)
		N.reagents.trans_to(hit_atom, N.reagents.total_volume, safety = 1)
		hit_atom:reagents.handle_reactions(shock=1)
		del(N)
	else
		src.reagents.handle_reactions(shock=1)
		del(N)